<div class="form-group">
  ...
  <select class="form-control" name="market">
    @foreach(config('classification.markets') as $market => $name)
      <option value="{{ $market }}" {{ isset($plan->market) ? ($plan->market == $market ? 'selected' : '') : (isset($ask->market)?($ask->market == $market ? 'selected' : ''):'')}}>{{ $name }}</option>
    @endforeach
  </select>
</div>
<div class="form-group">
  <label for="symbol"><span class="text-muted">Symbol</span></label>
  <input class="form-control" type="text" name="symbol" value="{{ old('symbol', $plan->symbol??($ask->symbol??'') ) }}" placeholder="" />
</div>
<div class="form-group">
  <label for="name"><span class="text-muted">名称</span></label>
  <input class="form-control" type="text" name="name" value="{{ old('name', $plan->name??($ask->name??'') ) }}" placeholder="可选" />
</div>
<div class="form-group">
  <label for="period"><span class="text-muted">周期</span></label>
  <select class="form-control" name="period">
    @foreach(config('classification.periods') as $period)
      <option value="{{ $period }}" {{ isset($plan->period)?($plan->period == $period ? 'selected' : ''): ( isset($ask->period)?($ask->period == $period ? 'selected':'') : '')  }}>{{ $period }}</option>
    @endforeach
  </select>
</div>
<div class="form-group">
  <label for="period"><span class="text-danger"><span class="text-muted">总资金</span></span></label>
  <input class="form-control" type="text" name="total" value="{{ old('total', $plan->total??($ask->total??'') ) }}" placeholder="" />
</div>
<div class="form-group">
  ...
  <select class="form-control" name="lever">
    @foreach(config('classification.levers') as $lever => $name)
      <option value="{{ $lever }}" {{ isset($plan->lever)?($plan->lever == $lever ? 'selected' : ''):(isset($ask->lever)?($ask->lever == $lever ? 'selected' : ''):'' ) }}>{{ $name }}</option>
    @endforeach
  </select>
</div>
<div class="form-group">
